...
#pragma omp for schedule(dynamic)
  for (int i = 0; i < contours.size(); ++i)
  {
    ...
    for (int j = 0; j < contour.size(); ++j)
    {
      ...
      cv::Mat confidencePatch = getPatch(confidenceMat, point);

      // get confidence of patch
      double confidence = cv::sum(confidencePatch)[0] / (double) confidencePatch.total();
      assert(0 <= confidence && confidence <= 1.0f);

      // get the normal to the border around point
      cv::Point2f normal = getNormal(contour, point);

      // get the maximum gradient in source around patch
      cv::Mat magnitudePatch = getPatch(maskedMagnitude, point);
      cv::Point maxPoint;
      cv::minMaxLoc(magnitudePatch, NULL, NULL, NULL, &maxPoint);
      cv::Point2f gradient = cv::Point2f(
