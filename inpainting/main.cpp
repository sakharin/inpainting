...
int main (int argc, char** argv) {
  ...
  while (cv::countNonZero(maskMat) != area) // end when target is filled
  {
    ...
    cv::Mat img_contour = cv::Mat::zeros(colorMat.size(), CV_8UC3);
    int contourIdx = -1;
    cv::Scalar color(0, 0, 255);
    cv::drawContours(img_contour, contours, contourIdx, color);
    cv::imshow("Bg2:, contour", img_contour);
    ...
    cv::imshow("Bg1", colorMat);
    cv::waitKey(1);
    std::cout << colorMat.rows * colorMat.cols - cv::countNonZero(maskMat) << " remaining pixels.\r" << std::flush;
  }
